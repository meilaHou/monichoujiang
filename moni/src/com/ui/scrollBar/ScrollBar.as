...
package com.ui.scrollBar
{
	...
	public class ScrollBar implements IUI
	{
		...
		*/  
		...
		private function checkAbled():void
		{
			...
			}else
			{
				...
				timeListener();
				_target.addEventListener(MouseEvent.MOUSE_OVER, handleMousIn);
				_target.addEventListener(MouseEvent.MOUSE_OUT, handleMousOut);
			}
		}
		/**
		 * 解决鼠标滚轮的冲突
		...
		{
			_scrollBar.removeEventListener(MouseEvent.MOUSE_DOWN,              scrollBarMouseDownHandler, false );
			_scrollBar.removeEventListener(MouseEvent.MOUSE_UP,                scrollBarMouseUpHandler, false);
			_scrollBar.parent.stage.removeEventListener(MouseEvent.MOUSE_UP,   scrollBarMouseUpHandler, false);
			_scrollBar.root.stage.removeEventListener(Event.MOUSE_LEAVE,       scrollBarMouseUpHandler, false);
			_scrollBar.x = _scrollBarOriginalPoint.x;
			_scrollBar.y = _scrollBarOriginalPoint.y;
			_target.removeEventListener(MouseEvent.MOUSE_OVER, handleMousIn);
			_target.removeEventListener(MouseEvent.MOUSE_OUT, handleMousOut);
		}
	}
}