...
package com.ui.text
{
	...
	internal class SpriteRenderer
	{
		...
		{			
			...
			if (rect != null)
			{
				rect.height = textRenderer.defaultTextFormat.size as Number;
...
//				trace("yyyyy"+_spriteContainer.y)
			}
		}
		...
		internal function exportXML():XML
		{
			...
			for (var s:* in _spriteIndices)
			{
				var info:Object = { src:getQualifiedClassName(s), index:s.name ,htmlText:""};
				if(s is TextField)
				{
					info.htmlText =  s.htmlText;
				}
				arr.push(info);
			}
			...
			{
				var node:XML = <sprite src={arr[i].src} index={arr[i].index - i} htmlText={arr[i].htmlText} />;
