...
package com.ui.text
{
	...
	import com.ui.text.plugins.IRTFPlugin;
	import com.panels.chat.MaleIcon;
...
//		private static var isFirst1:Boolean=true;
...
//					_textRenderer.htmlText += "<p>" + newText + "</p>";	
		...
		private function insertSprites(newSprites:Array, startIndex:int, maxIndex:int, newline:Boolean = false):void
		{
			...
			{
				if(newSprites[0].index != newSprites[1].index)
				{
			...
			for (var i:int = 0; i < newSprites.length; i++)
			{
				...
				if (obj.index == undefined || index < 0 || index > maxIndex - startIndex+1) //+1
				{
		...
		{
			if (_width == awidth && _height == aheight) return;
			_width = awidth;
			_height = aheight;
